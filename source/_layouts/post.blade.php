...
@section('body')
    <div class="w-1/2 mx-auto">
        <h1 class="inline-block text-3xl font-bold text-gray-900">{{ $page->title }}</h1>
        ...
        <div class="flex items-center justify-between py-5 my-5 border-b-2 border-gray-100 w-full">
            ...
            @endif
            <div class="">
                <div class="ya-share2" data-curtain data-shape="square" data-color-scheme="whiteblack" data-services="vkontakte,facebook,telegram,twitter"></div>
            </div>
        </div>
        ...
        @if($original = $page->original)
            <div class="w-full text-white bg-gray-400 my-5">
                <div class="container flex items-center justify-between px-6 py-4 mx-auto">
                    <div class="flex">
                        Перевод статьи «<a href="{{ $original['url'] }}" rel="noreferrer noopener" class="alert-link" target="_blank">{{ $original['title'] }}</a>»
                    </div>
                </div>
            </div>
        @endif
        
        <img class="img-fluid w-100 mb-2 rounded" src="{{ $page->getImage() }}" alt="{{ $page->title }}" title="{{ $page->title }}"/>
